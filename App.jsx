...
var Stat = function({ label, value, sub, accent }) { return (
...
var Modal = function({ title, onClose, children }) { return (
  <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }} onClick={onClose}>
    <div onClick={function(e){e.stopPropagation()}} style={{ background: C.card, borderRadius: 16, border: "1px solid " + C.border, padding: 28, width: 560, maxWidth: "95vw", maxHeight: "90vh", overflow: "auto" }}>
...
var Tbl = function({ columns, data, actions }) { return (
  <div style={{ overflowX: "auto", borderRadius: 12, border: "1px solid " + C.border }}>
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
      <thead><tr>
        {columns.map(function(c){return <th key={c.key} style={{ textAlign: "left", padding: "12px 14px", background: C.bg, color: C.textDim, fontWeight: 600, fontSize: 10, textTransform: "uppercase", letterSpacing: 1, borderBottom: "1px solid " + C.border, whiteSpace: "nowrap" }}>{c.label}</th>})}
        {actions && <th style={{ textAlign: "right", padding: "12px 14px", background: C.bg, color: C.textDim, fontWeight: 600, fontSize: 10, letterSpacing: 1, borderBottom: "1px solid " + C.border }}>Actions</th>}
      </tr></thead>
      <tbody>{data.map(function(row, i){return (
        <tr key={row.id || i} style={{ borderBottom: "1px solid " + C.border }}>
          {columns.map(function(c){return <td key={c.key} style={{ padding: "10px 14px", color: C.text, whiteSpace: "nowrap" }}>{c.render ? c.render(row[c.key], row) : row[c.key]}</td>})}
...
var Header = function({ children, right }) { return (
...
var Toast = function({ msg, type, onClose }) { return (
  <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 2000, background: type === "error" ? C.red : C.green, color: "#fff", padding: "12px 24px", borderRadius: 10, fontWeight: 600, fontSize: 13, display: "flex", alignItems: "center", gap: 10, boxShadow: "0 8px 30px rgba(0,0,0,.4)" }}>
    {msg}
    <button onClick={onClose} style={{ background: "none", border: "none", color: "#fff", cursor: "pointer" }}><Ico d={ICON.x} s={14} /></button>
  </div>
); };
...
// Helper: get counts for a customer on a date
...
function LoginScreen({ onLogin }) {
  ...
  var handleSubmit = async function() {
    ...
    try {
      var result;
      if (isSignup) { result = await supabase.auth.signUp({ email: email, password: password }); }
      else { result = await supabase.auth.signInWithPassword({ email: email, password: password }); }
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: C.bg, fontFamily: "'DM Sans', sans-serif", padding: 20 }}>
      <div style={{ background: C.card, borderRadius: 20, border: "1px solid " + C.border, padding: 40, width: 400, maxWidth: "95vw" }}>
        ...
        <Btn primary disabled={loading} onClick={handleSubmit} style={{ width: "100%", justifyContent: "center", padding: "12px 0", fontSize: 14, marginBottom: 12 }}>
          <Ico d={ICON.lock} s={16} /> {loading ? "Please wait..." : isSignup ? "Create Account" : "Sign In"}
        </Btn>
        <div style={{ textAlign: "center" }}>
          <button onClick={function(){ setIsSignup(!isSignup); setError(""); }} style={{ background: "none", border: "none", color: C.accent, cursor: "pointer", fontSize: 13, fontFamily: "inherit" }}>
...
// =====================
// OPERATIONS (proper component for hooks)
// =====================
function OperationsTab({ customers, overrides, selDate, setSelDate, onSaveOverride, onRemoveOverride }) {
  ...
  var startEdit = function(row) {
    setEditId(row.id);
    setEditVeg(row.counts.veg);
    setEditNV(row.counts.nonveg);
    setEditCh(row.counts.chapathi);
  };

  var saveEdit = function(id) {
    onSaveOverride(id, selDate, parseInt(editVeg)||0, parseInt(editNV)||0, parseInt(editCh)||0);
    setEditId(null);
  };

  var numInput = function(val, setter, color) {
    return <input type="number" value={val} onChange={function(e){setter(e.target.value)}} style={{ width: 50, padding: "4px 6px", borderRadius: 6, border: "1px solid " + color, background: C.bg, color: color, fontSize: 13, outline: "none", fontFamily: "'DM Mono', monospace", textAlign: "center" }} />;
  };
  ...
  return (
    <div>
      ...
      <Tbl columns={[
        ...
        { key: "counts", label: "Veg", render: function(v, row) {
          if (editId === row.id) return numInput(editVeg, setEditVeg, C.green);
          return <span style={{ fontWeight: 700, fontFamily: "'DM Mono', monospace", color: C.green }}>{v.veg}</span>;
        }},
        { key: "_nv", label: "Non-Veg", render: function(v, row) {
          if (editId === row.id) return numInput(editNV, setEditNV, C.red);
          return <span style={{ fontWeight: 700, fontFamily: "'DM Mono', monospace", color: C.red }}>{row.counts.nonveg}</span>;
        }},
        { key: "_ch", label: "Chapathi", render: function(v, row) {
          if (editId === row.id) return numInput(editCh, setEditCh, C.accent);
          return <span style={{ fontWeight: 700, fontFamily: "'DM Mono', monospace", color: C.accent }}>{row.counts.chapathi}</span>;
        }},
        ...
        if (editId === row.id) return (
          <div style={{ display: "flex", gap: 4 }}>
            <Btn small primary onClick={function(){ saveEdit(row.id); }}><Ico d={ICON.check} s={14} /></Btn>
            <Btn small onClick={function(){ setEditId(null); }}><Ico d={ICON.x} s={14} /></Btn>
          </div>
        );
        return (
          <div style={{ display: "flex", gap: 4 }}>
            <Btn small onClick={function(){ startEdit(row); }}><Ico d={ICON.edit} s={14} /></Btn>
            {isOv && <Btn small danger onClick={function(){ onRemoveOverride(row.id, selDate); }}>Reset</Btn>}
          </div>
        );
      }} />
      <div style={{ marginTop: 16, padding: 16, background: C.card, borderRadius: 10, border: "1px solid " + C.border, fontSize: 12, color: C.textDim }}>
        Click the edit icon to change meal counts for any customer on this date (today or future). Click Reset to revert to defaults.
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(function() {
    supabase.auth.getSession().then(function(resp) {
      setUser(resp.data.session ? resp.data.session.user : null);
  ...
  // Kitchen data: aggregate all customer counts for selected date
  var kitchenData = useMemo(function() {
    ...
    customers.forEach(function(c) {
      ...
      d.veg += counts.veg;
      d.nonveg += counts.nonveg;
      d.chapathi += counts.chapathi;
      d.total += t;
    ...
    try {
      if (editCust && editCust.id && customers.find(function(c){return c.id===editCust.id})) {
        ...
        var insRes = await supabase.from("customers").insert({
          ...
          meal_type: "Veg", billing_cycle: data.billing_cycle, route: data.route, balance: 0, last_billed: today,
        });
  ...
  var updatePricing = async function(np) {
    try {
      for (var i = 0; i < MEAL_TYPES.length; i++) {
        await supabase.from("pricing").update({ price: np[MEAL_TYPES[i]], updated_at: new Date().toISOString() }).eq("meal_type", MEAL_TYPES[i]);
      }
  ...
  var sendWhatsApp = function(route) {
    ...
    list.forEach(function(c, i) {
      ...
      msg += (i+1) + ". " + c.name + "\nAddress: " + c.address + "\nPhone: " + c.phone1 + "\nMeals: " + parts.join(" + ") + " = " + c.totalMeals + " total\n";
    ...
    return (
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={{ gridColumn: "1/-1", padding: "12px 16px", background: C.bg, borderRadius: 10, border: "1px solid " + C.border }}>
          ...
          <div style={{ marginTop: 8, fontSize: 12, color: C.accent, fontWeight: 600 }}>Total per day: {totalDef} meals</div>
        </div>
  ...
  var InvForm = function({ onSave, onCancel }) {
    ...
    return (
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        {cust && <div style={{ gridColumn: "1/-1", color: C.textDim, fontSize: 12 }}>{cust.name} | Defaults: {cust.default_veg||0}V + {cust.default_nonveg||0}NV + {cust.default_chapathi||0}Ch</div>}
  ...
  var PriceForm = function({ onSave, onCancel }) {
    ...
    return (
      <div>
        {MEAL_TYPES.map(function(t){return <Inp key={t} label={t + " per meal"} value={p[t]} onChange={function(v){setP(function(pr){return Object.assign({},pr,{[t]:parseInt(v)||0})})}} type="number" style={{ marginBottom: 16 }} />})}
        <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
          <Btn onClick={onCancel}>Cancel</Btn>
          <Btn primary onClick={function(){onSave(p)}}>Save</Btn>
        </div>
      </div>
    );
  };
    ...
    return (
      <div>
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700 }}>Welcome to Mullai</h1>
        ...
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 24 }}>
          <Stat label="Total Meals Today" value={kitchenData.total} sub={kitchenData.veg + " Veg | " + kitchenData.nonveg + " NV | " + kitchenData.chapathi + " Ch"} accent={C.accent} />
          <Stat label="Active Customers" value={customers.length} sub={Object.keys(routeGroups).length + " routes"} />
          <Stat label="Pending Payments" value={toINR(pendingTotal)} sub={invoices.filter(function(i){return i.status==="Unpaid"}).length + " invoices"} accent={C.red} />
          <Stat label="Revenue Today" value={toINR(todayRev)} accent={C.green} />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: window.innerWidth > 768 ? "1fr 1fr" : "1fr", gap: 20 }}>
          <Card>
            ...
            <div style={{ display: "flex", gap: 12 }}>
              {[{l:"Veg",v:kitchenData.veg,c:C.green},{l:"Non-Veg",v:kitchenData.nonveg,c:C.red},{l:"Chapathi",v:kitchenData.chapathi,c:C.accent}].map(function(m){
                var pct = kitchenData.total ? Math.round(m.v / kitchenData.total * 100) : 0;
                return <div key={m.l} style={{ flex:1, background:m.c+"12", borderRadius:12, padding:16, textAlign:"center" }}>
                  <div style={{ fontSize:24, fontWeight:700, color:m.c, fontFamily:"'DM Mono', monospace" }}>{m.v}</div>
                  <div style={{ fontSize:11, color:C.textDim, marginTop:4 }}>{m.l} ({pct}%)</div>
                </div>;
              })}
            </div>
            <div style={{ marginTop: 16, height: 8, borderRadius: 4, background: C.bg, display: "flex", overflow: "hidden" }}>
              {[{v:kitchenData.veg,c:C.green},{v:kitchenData.nonveg,c:C.red},{v:kitchenData.chapathi,c:C.accent}].map(function(m,i){
                var pct = kitchenData.total ? m.v/kitchenData.total*100 : 0;
                return <div key={i} style={{ width:pct+"%", background:m.c }} />;
              })}
            </div>
          </Card>
          <Card>
            ...
            {Object.keys(routeGroups).length === 0 && <p style={{ color: C.textDim, fontSize: 13 }}>Add customers to see routes</p>}
            {Object.entries(routeGroups).map(function(entry){ var route=entry[0]; var list=entry[1]; return (
              <div key={route} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 0", borderBottom:"1px solid "+C.border }}>
                ...
                <span style={{ fontWeight:700, fontFamily:"'DM Mono', monospace", color:C.accent }}>{list.reduce(function(s,c){return s+c.totalMeals},0)}</span>
              </div>
            ); })}
          </Card>
        </div>
        {invoices.filter(function(i){return i.status==="Unpaid"}).length > 0 && (
          <Card style={{ marginTop: 20 }}>
            ...
            ]} data={invoices.filter(function(i){return i.status==="Unpaid"})} actions={function(row){return <Btn small primary onClick={function(){markPaid(row.id)}}>Paid</Btn>}} />
          </Card>
        )}
      </div>
    );
      ...
      <div>
        <Header right={<>
          <div style={{ position: "relative" }}>
            <input value={searchQ} onChange={function(e){setSearchQ(e.target.value)}} placeholder="Search..." style={{ padding: "9px 12px 9px 36px", borderRadius: 8, border: "1px solid " + C.border, background: C.bg, color: C.text, fontSize: 13, outline: "none", width: 200, fontFamily: "inherit" }} />
            <span style={{ position: "absolute", left: 10, top: 9, color: C.textDim }}><Ico d={ICON.search} s={16} /></span>
          </div>
          <Btn primary onClick={function(){setEditCust(null);setModal("customer")}}><Ico d={ICON.plus} s={14} /> Add</Btn>
        </>}>Customer Management</Header>
        <Tbl columns={[
          { key: "id", label: "ID", render: function(v){return <Badge>{v}</Badge>} },
          { key: "name", label: "Office", render: function(v){return <span style={{fontWeight:600}}>{v}</span>} },
          { key: "route", label: "Route", render: function(v){return <Badge color={C.blue}>{v}</Badge>} },
          { key: "default_veg", label: "Veg", render: function(v){return <span style={{fontWeight:700,color:C.green,fontFamily:"'DM Mono', monospace"}}>{v||0}</span>} },
  ...
  var renderKitchen = function() { return (
    <div>
      <Header right={<>
        <Inp value={selDate} onChange={setSelDate} type="date" />
        <Btn onClick={function(){exportCSV("kitchen_"+selDate+".csv", ["ID","Office","Veg","Non-Veg","Chapathi","Total","Route"], kitchenData.details.map(function(c){return [c.id,c.name,c.counts.veg,c.counts.nonveg,c.counts.chapathi,c.totalMeals,c.route]}))}}><Ico d={ICON.dl} s={14} /> Export</Btn>
      ...
      <Tbl columns={[
        ...
        { key: "_nv", label: "Non-Veg", render: function(v,row){return <span style={{fontWeight:700,color:C.red,fontFamily:"'DM Mono', monospace"}}>{row.counts.nonveg}</span>} },
        { key: "_ch", label: "Chapathi", render: function(v,row){return <span style={{fontWeight:700,color:C.accent,fontFamily:"'DM Mono', monospace"}}>{row.counts.chapathi}</span>} },
  ...
  var renderBilling = function() { return (
    <div>
      <Header right={<>
        <Btn onClick={function(){setModal("pricing")}}><Ico d={ICON.money} s={14} /> Pricing</Btn>
        <Btn onClick={function(){setModal("invoice")}}><Ico d={ICON.plus} s={14} /> Invoice</Btn>
        <Btn onClick={function(){exportCSV("invoices.csv", ["Invoice","Customer","Period","Amount","Status"], invoices.map(function(i){return [i.id,i.customer_id,i.period_label,i.amount,i.status]}))}}><Ico d={ICON.dl} s={14} /> Export</Btn>
      </>}>Billing</Header>
      <div style={{ display: "flex", gap: 16, marginBottom: 24, flexWrap: "wrap" }}>
      ...
      ]} data={invoices} actions={function(row){return row.status==="Unpaid" ? <Btn small primary onClick={function(){markPaid(row.id)}}>Paid</Btn> : <Badge color={C.green}>Done</Badge>}} />
  ...
  var renderDelivery = function() { return (
    <div>
      <Header right={<>
        <Inp value={selDate} onChange={setSelDate} type="date" />
        <Inp value={routeFilter} onChange={setRouteFilter} options={["All"].concat(ROUTES)} />
      ...
      {Object.entries(routeGroups).filter(function(e){return routeFilter==="All"||e[0]===routeFilter}).map(function(entry){var route=entry[0]; var list=entry[1]; return (
        <Card key={route} style={{ marginBottom: 20 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16, flexWrap:"wrap", gap:10 }}>
            <div><span style={{ fontSize:16, fontWeight:700 }}>{route}</span> <Badge color={C.blue}>{list.length + " stops | " + list.reduce(function(s,c){return s+c.totalMeals},0) + " meals"}</Badge></div>
          ...
          {list.map(function(c, i){return (
            <div key={c.id} style={{ display:"flex", alignItems:"center", gap:16, padding:"14px 0", borderTop: i ? "1px solid "+C.border : "none" }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ fontSize:12, color:C.textDim }}>{c.address}</div>
                <div style={{ fontSize:12, color:C.textMid, marginTop:2 }}>{c.phone1}{c.map_link ? " | " : ""}{c.map_link ? <a href={c.map_link} target="_blank" rel="noreferrer" style={{ color:C.blue, textDecoration:"none" }}>Map</a> : null}</div>
              </div>
              <div style={{ textAlign:"right", flexShrink:0 }}>
                <div style={{ fontWeight:700, fontSize:18, fontFamily:"'DM Mono', monospace", color:C.accent }}>{c.totalMeals}</div>
                <MealCounts counts={c.counts} />
              </div>
            </div>
          )})}
        </Card>
      ); })}
      {Object.keys(routeGroups).length === 0 && <Card><p style={{ color:C.textDim, textAlign:"center" }}>Add customers first</p></Card>}
    </div>
  ); };
    ...
    return (
      <div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 24, flexWrap: "wrap" }}>
          {RTYPES.map(function(r){return <Btn key={r} small primary={reportTab===r} onClick={function(){setReportTab(r)}}>{r}</Btn>})}
        </div>
        {reportTab === "Kitchen" && renderKitchen()}
        {reportTab === "Billing" && <Tbl columns={[
          { key: "id", label: "Invoice" }, { key: "customer_id", label: "Customer", render: function(v){var c=customers.find(function(x){return x.id===v}); return c?c.name:v;} },
          { key: "period_label", label: "Period" }, { key: "amount", label: "Amount", render: function(v){return toINR(v)} },
          { key: "status", label: "Status", render: function(v){return <Badge color={v==="Paid"?C.green:C.red}>{v}</Badge>} },
        ]} data={invoices} />}
        {reportTab === "Customer" && <Tbl columns={[
          { key: "id", label: "ID" }, { key: "name", label: "Name" }, { key: "route", label: "Route" },
          { key: "default_veg", label: "Veg", render: function(v){return v||0} },
          { key: "default_nonveg", label: "NV", render: function(v){return v||0} },
          { key: "default_chapathi", label: "Ch", render: function(v){return v||0} },
          { key: "billing_cycle", label: "Cycle" },
        ]} data={customers} />}
        {reportTab === "Delivery" && <div><Inp value={selDate} onChange={setSelDate} type="date" style={{ marginBottom: 16, maxWidth: 200 }} />
          <Tbl columns={[{ key: "id", label: "ID" }, { key: "name", label: "Office" }, { key: "route", label: "Route" },
            { key: "counts", label: "Meals", render: function(v){return <MealCounts counts={v} />} },
            { key: "totalMeals", label: "Total", render: function(v){return <span style={{fontWeight:700,fontFamily:"'DM Mono', monospace"}}>{v}</span>} }]} data={kitchenData.details} /></div>}
        {reportTab === "Route" && Object.entries(routeGroups).map(function(entry){var route=entry[0]; var list=entry[1]; return (
          <Card key={route} style={{ marginBottom: 16 }}>
            <div style={{ fontWeight:700, marginBottom:12 }}>{route + " - " + list.length + " customers | " + list.reduce(function(s,c){return s+c.totalMeals},0) + " meals"}</div>
            <Tbl columns={[{ key: "name", label: "Office" }, { key: "counts", label: "Meals", render: function(v){return <MealCounts counts={v} />} }, { key: "totalMeals", label: "Total" }]} data={list} />
          </Card>
        ); })}
  ...
  var pages = {
    ...
    Operations: function(){ return <OperationsTab customers={customers} overrides={overrides} selDate={selDate} setSelDate={setSelDate} onSaveOverride={saveOverride} onRemoveOverride={removeOverride} />; },
      ...
      <div style={{ width: sidebar?220:56, background:C.card, borderRight:"1px solid "+C.border, display:"flex", flexDirection:"column", transition:"width .2s", flexShrink:0, overflow:"hidden" }}>
        ...
        <nav style={{ padding:"12px 8px", flex:1 }}>
          {TABS.map(function(t){return (
            <div key={t} onClick={function(){setTab(t); if(window.innerWidth<768)setSidebar(false)}} style={{
        ...
        <div style={{ padding: sidebar?"12px 16px":"12px 8px", borderTop:"1px solid "+C.border }}>
          <div onClick={handleLogout} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px", borderRadius:10, cursor:"pointer", color:C.textDim, justifyContent: sidebar?"flex-start":"center" }}>
            <Ico d={ICON.logout} s={18} /> {sidebar && <span style={{ fontSize:13 }}>Logout</span>}
          </div>
