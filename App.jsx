...
const Modal = ({ title, onClose, children }) => (
  <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }} onClick={onClose}>
    <div onClick={e => e.stopPropagation()} style={{ background: C.card, borderRadius: 16, border: "1px solid " + C.border, padding: 28, width: 500, maxWidth: "95vw", maxHeight: "90vh", overflow: "auto" }}>
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: C.bg, fontFamily: "'DM Sans', sans-serif", padding: 20 }}>
      <div style={{ background: C.card, borderRadius: 20, border: "1px solid " + C.border, padding: 40, width: 400, maxWidth: "95vw" }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <Inp label="Password" value={password} onChange={setPassword} type="password" placeholder="Min 6 characters" />
        </div>
        ...
        <div style={{ textAlign: "center" }}>
          <button onClick={() => { setIsSignup(!isSignup); setError(""); }} style={{ background: "none", border: "none", color: C.accent, cursor: "pointer", fontSize: 13, fontFamily: "inherit" }}>
            {isSignup ? "Already have an account? Sign In" : "First time? Create Account"}
          </button>
        </div>
        <div style={{ marginTop: 24, padding: "12px 16px", background: C.bg, borderRadius: 10, fontSize: 11, color: C.textDim, lineHeight: 1.6 }}>
          Your data is encrypted and secured by Supabase. Only you can access your dashboard after signing in.
        </div>
      </div>
    </div>
  );
...
function OperationsTab({ customers, overrides, selDate, setSelDate, getMealCount, onSaveOverride, onRemoveOverride }) {
  const [editId, setEditId] = useState(null);
  const [editVal, setEditVal] = useState("");

  const details = useMemo(() => {
    return customers.map(c => ({ ...c, todayMeals: getMealCount(c, selDate) }));
  ...
  return (
    <div>
      ...
      <Tbl columns={[
        ...
        { key: "todayMeals", label: "Today Count", render: (v, row) => {
          var isOv = overrides.some(o => o.customer_id === row.id && o.override_date === selDate);
          if (editId === row.id) return (
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              <input autoFocus type="number" value={editVal} onChange={e => setEditVal(e.target.value)}
                style={{ width: 70, padding: "6px 10px", borderRadius: 6, border: "1px solid " + C.accent, background: C.bg, color: C.text, fontSize: 14, outline: "none", fontFamily: "inherit" }}
                onKeyDown={e => { if (e.key === "Enter") { onSaveOverride(row.id, selDate, parseInt(editVal) || 0); setEditId(null); } if (e.key === "Escape") setEditId(null); }} />
              <Btn small primary onClick={() => { onSaveOverride(row.id, selDate, parseInt(editVal) || 0); setEditId(null); }}><Ico d={ICON.check} s={14} /></Btn>
      ...
      ]} data={details} actions={row => {
        var isOv = overrides.some(o => o.customer_id === row.id && o.override_date === selDate);
        return isOv ? <Btn small danger onClick={() => onRemoveOverride(row.id, selDate)}>Reset</Btn> : null;
      }} />
      <div style={{ marginTop: 16, padding: 16, background: C.card, borderRadius: 10, border: "1px solid " + C.border, fontSize: 12, color: C.textDim }}>
        Click on any meal count to change it for this date. Click Reset to go back to the default.
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  var showToast = (msg, type) => { setToast({ msg: msg, type: type || "success" }); setTimeout(() => setToast(null), 3000); };
  ...
  var loadAll = useCallback(async () => {
    ...
    try {
      ...
      if (results[1].data) {
        var p = {};
        results[1].data.forEach(function(r) { p[r.meal_type] = r.price; });
        setPrices(p);
      }
  ...
  var getMealCount = useCallback((cust, date) => {
    var ov = overrides.find(o => o.customer_id === cust.id && o.override_date === date);
    return ov ? ov.meal_count : cust.default_meals;
  }, [overrides]);

  var kitchenData = useMemo(() => {
    var d = { Veg: 0, "Non-Veg": 0, Chapathi: 0, total: 0, details: [] };
    customers.forEach(c => {
      var count = getMealCount(c, selDate);
      d[c.meal_type] += count;
      d.total += count;
      d.details.push(Object.assign({}, c, { todayMeals: count }));
    });
    return d;
  }, [customers, selDate, getMealCount]);

  var routeGroups = useMemo(() => {
    var g = {};
    customers.forEach(c => {
      if (!g[c.route]) g[c.route] = [];
      g[c.route].push(Object.assign({}, c, { todayMeals: getMealCount(c, selDate) }));
    });
    return g;
  }, [customers, selDate, getMealCount]);

  var pendingTotal = invoices.filter(i => i.status === "Unpaid").reduce((s, i) => s + Number(i.amount), 0);

  var saveCustomer = async (data) => {
    try {
      if (editCust && editCust.id && customers.find(c => c.id === editCust.id)) {
  ...
  var saveOverride = async (custId, date, count) => {
    try {
  ...
  var updatePricing = async (np) => {
    try {
      for (var i = 0; i < MEAL_TYPES.length; i++) {
        var mt = MEAL_TYPES[i];
        await supabase.from("pricing").update({ price: np[mt], updated_at: new Date().toISOString() }).eq("meal_type", mt);
      }
      setPrices(np); showToast("Pricing updated!"); setModal(null);
    } catch (e) { showToast("Error: " + e.message, "error"); }
  };

  var sendWhatsApp = (route) => {
    var list = routeGroups[route] || [];
    var msg = "MULLAI DELIVERIES\n" + selDate + " | " + route + "\n============================\n\n";
    list.forEach((c, i) => {
      msg += (i + 1) + ". " + c.name + "\nAddress: " + c.address + "\nPhone: " + c.phone1 + "\nMeals: " + c.todayMeals + " x " + c.meal_type + "\n";
  ...
  var CustForm = function({ initial, onSave, onCancel }) {
    var empty = { name: "", address: "", phone1: "", phone2: "", map_link: "", default_meals: 10, meal_type: "Veg", billing_cycle: "30 Days", route: "Route 1" };
    var init = initial ? { name: initial.name, address: initial.address, phone1: initial.phone1, phone2: initial.phone2 || "", map_link: initial.map_link || "", default_meals: initial.default_meals, meal_type: initial.meal_type, billing_cycle: initial.billing_cycle, route: initial.route } : empty;
    var [f, setF] = useState(init);
    var set = (k, v) => setF(p => Object.assign({}, p, { [k]: v }));
    return (
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <Inp label="Default Meals" value={f.default_meals} onChange={v => set("default_meals", parseInt(v) || 0)} type="number" />
        ...
        <div style={{ gridColumn: "1/-1", display: "flex", gap: 12, justifyContent: "flex-end", marginTop: 8 }}>
          ...
          <Btn primary onClick={() => onSave(f)} disabled={!f.name || !f.phone1}>Save</Btn>
        </div>
      </div>
    );
  };
  ...
  var InvForm = function({ onSave, onCancel }) {
    ...
    var set = (k, v) => setF(p => Object.assign({}, p, { [k]: v }));
    var cust = customers.find(c => c.id === f.customer_id);
    return (
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={{ gridColumn: "1/-1", display: "flex", gap: 12, justifyContent: "flex-end", marginTop: 8 }}>
          <Btn onClick={onCancel}>Cancel</Btn>
          <Btn primary onClick={() => onSave({ id: "INV-" + Date.now().toString(36).toUpperCase(), customer_id: f.customer_id, period_start: f.period_start, period_end: f.period_end, period_label: f.period_start + " to " + f.period_end, amount: f.amount, status: "Unpaid" })}>Create</Btn>
        </div>
      </div>
    );
  };
  ...
  var PriceForm = function({ onSave, onCancel }) {
    ...
    return (
      <div>
        {MEAL_TYPES.map(t => <Inp key={t} label={t + " per meal"} value={p[t]} onChange={v => setP(pr => Object.assign({}, pr, { [t]: parseInt(v) || 0 }))} type="number" style={{ marginBottom: 16 }} />)}
        <div style={{ display: "flex", gap: 12, justifyContent: "flex-end", marginTop: 8 }}>
          <Btn onClick={onCancel}>Cancel</Btn>
          <Btn primary onClick={() => onSave(p)}>Save</Btn>
        </div>
      </div>
    );
  };
  ...
  var renderDashboard = () => (
    <div>
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 24 }}>
        ...
        <Stat label="Revenue Today" value={toINR(kitchenData.details.reduce((s, c) => s + c.todayMeals * (prices[c.meal_type] || 0), 0))} accent={C.green} />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: window.innerWidth > 768 ? "1fr 1fr" : "1fr", gap: 20 }}>
        <Card>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            {MEAL_TYPES.map(t => {
              var pct = kitchenData.total ? Math.round(kitchenData[t] / kitchenData.total * 100) : 0;
              var clr = t === "Veg" ? C.green : t === "Non-Veg" ? C.red : C.accent;
          ...
          <div style={{ marginTop: 16, height: 8, borderRadius: 4, background: C.bg, display: "flex", overflow: "hidden" }}>
            {MEAL_TYPES.map(t => { var pct = kitchenData.total ? kitchenData[t] / kitchenData.total * 100 : 0; var clr = t === "Veg" ? C.green : t === "Non-Veg" ? C.red : C.accent; return <div key={t} style={{ width: pct + "%", background: clr }} />; })}
          </div>
        </Card>
        <Card>
      ...
      {invoices.filter(i => i.status === "Unpaid").length > 0 && (
        <Card style={{ marginTop: 20 }}>
          ...
          <Tbl columns={[
            ...
            { key: "customer_id", label: "Customer", render: v => { var c = customers.find(x => x.id === v); return c ? c.name : v; } },
  ...
  var renderCustomers = () => {
    var filtered = customers.filter(c => !searchQ || c.name.toLowerCase().includes(searchQ.toLowerCase()) || c.id.toLowerCase().includes(searchQ.toLowerCase()));
    return (<div>
      <Header right={<>
        ...
        <Btn primary onClick={() => { setEditCust(null); setModal("customer"); }}><Ico d={ICON.plus} s={14} /> Add</Btn>
      ...
      {modal === "customer" && <Modal title={editCust && editCust.id ? "Edit " + editCust.name : "Add Customer"} onClose={() => { setModal(null); setEditCust(null); }}>
        <CustForm initial={editCust} onSave={saveCustomer} onCancel={() => { setModal(null); setEditCust(null); }} />
      </Modal>}
    </div>);
  };

  var renderKitchen = () => (<div>
    <Header right={<>
  ...
  var renderBilling = () => (<div>
    <Header right={<>
      <Btn onClick={() => setModal("pricing")}><Ico d={ICON.money} s={14} /> Pricing</Btn>
      <Btn onClick={() => setModal("invoice")}><Ico d={ICON.plus} s={14} /> Invoice</Btn>
    ...
    ]} data={invoices} actions={row => row.status === "Unpaid" ? <Btn small primary onClick={() => markPaid(row.id)}>Paid</Btn> : <Badge color={C.green}>Done</Badge>} />
    {modal === "pricing" && <Modal title="Pricing" onClose={() => setModal(null)}><PriceForm onSave={updatePricing} onCancel={() => setModal(null)} /></Modal>}
    {modal === "invoice" && <Modal title="New Invoice" onClose={() => setModal(null)}><InvForm onSave={createInvoice} onCancel={() => setModal(null)} /></Modal>}
  </div>);

  var renderDelivery = () => (<div>
    <Header right={<>
      <Inp value={selDate} onChange={setSelDate} type="date" />
      <Inp value={routeFilter} onChange={setRouteFilter} options={["All"].concat(ROUTES)} />
    </>}>{"Delivery - " + selDate}</Header>
    {Object.entries(routeGroups).filter(entry => routeFilter === "All" || entry[0] === routeFilter).map(function(entry) { var route = entry[0]; var list = entry[1]; return (
  ...
  var renderReports = () => {
    ...
    return (<div>
      ...
      {reportTab === "Billing" && <Tbl columns={[
        { key: "id", label: "Invoice" }, { key: "customer_id", label: "Customer", render: v => { var c = customers.find(x => x.id === v); return c ? c.name : v; } },
      ...
      {reportTab === "Delivery" && <div><Inp value={selDate} onChange={setSelDate} type="date" style={{ marginBottom: 16, maxWidth: 200 }} />
        <Tbl columns={[{ key: "id", label: "ID" }, { key: "name", label: "Office" }, { key: "route", label: "Route" }, { key: "meal_type", label: "Type" },
          { key: "todayMeals", label: "Meals", render: v => <span style={{ fontWeight: 700, fontFamily: "'DM Mono', monospace" }}>{v}</span> }]} data={kitchenData.details} /></div>}
  ...
  if (authLoading) return (
    <div style={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: C.bg, fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ textAlign: "center", color: C.textDim }}>Loading...</div>
    </div>
  );
  ...
  if (loading) return (
    <div style={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: C.bg, fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ textAlign: "center", color: C.textDim }}>Loading data...</div>
    </div>
  );
  ...
  var pages = {
    ...
    Operations: function() { return <OperationsTab customers={customers} overrides={overrides} selDate={selDate} setSelDate={setSelDate} getMealCount={getMealCount} onSaveOverride={saveOverride} onRemoveOverride={removeOverride} />; },
